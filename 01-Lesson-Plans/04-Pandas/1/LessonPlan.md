...
* The first step towards helping Harold decide which energy stock to include in the portfolio is to load the data files and concatenate all the data in a single DataFrame. Before using the pct_change() function, we need to ensure that the returns are sorted in ascending order, since this is what the function expects:

  ![sorted-dataframe](Images/sorted-dataframe.png)

* Then we calculate the daily returns using the `pct_change` function.
