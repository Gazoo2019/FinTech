...
* This lesson has a dependency on the Nasdaq Data Link and Alpaca APIs being up and running. Visit each site and execute preemptive API calls to ensure connectivity. It is imperative to confirm that the APIs used in this lesson are executing as expected.
...
* In preparation for next week's classes, slack out the [PyViz Installation Guide](../../06-PyViz/Supplemental/PyVizInstallationGuide.md). Tell students to complete the installations and verify them with a TA before the end of the next class.
...
* The slides for this lesson can be viewed on Google Drive here: [5.2 Slides](https://docs.google.com/presentation/d/1Jt8IVV3sd7OoO5VM_ssqX3d89IFISUd-OAryfxP0D6Y/edit?usp=sharing).
...
* You will gain hands-on experience on using Nasdaq Data Link (to fetch historical stock data) and Alpaca (a trading API)
...
* To set an environment variable, you have to define a name for that variable; let's start creating a variable to store the Nasdaq Data Link API key.

* Set the variable name as `NASDAQ_API_KEY` and assign your API key to it.

  ```text
  NASDAQ_API_KEY="ENTER YOUR API KEY HERE"
  ```
...
**Important Note:** Ensure that you have a local `.env` file containing your `NASDAQ_API_KEY` into the `Solved` folder before running the demo.
...
* If an environment variable does not exist, Python will return `NoneType` as the data type. An empty environment variable will cause an API call to fail.
  ...
  api_key_bis = os.getenv("MY_NASDAQ_API_KEY")
  type(api_key_bis)
  ```
...
* Once created, the environment variables are then shared with all child processes. For example, when the `load_dotenv()` method is executed, it will ensure the `NASDAQ_API_KEY` variable is accessible by all processes running in the environment that executed the python file process.

  ```shell
  NASDAQ_API_KEY="ENTER YOUR KEY HERE"
  ```
...
* Once stored as a Python variable, the environment variable value can be used for processing. In this case, the `NASDAQ_API_KEY` is stored as the Python variable `api_key` and then concatenated with the request URL. The concatenated request URL will then be used to submit a request to the Nasdaq API.
...
* If a user were to export the environment variable `NASDAQ_API_KEY` using a terminal but then launched Jupyter Lab in a different terminal window, would Jupyter Lab be able to retrieve the environment variable?

  **Answer:** No. Environment variables are sourced to current and child processes. Because a new terminal window is used to launch Jupyter Lab, the environment variable `NASDAQ_API_KEY` will be out of scope.
...
* Example companies that offer SDKs are Nasdaq, Alpaca, Investors Exchange, Google, and AWS.
...
* The following code depicts an example of the differences between using a request URL and an SDK.
  ...
  requests.get("https://data.nasdaq.com/api/v3/datasets/WIKI/AMD?api_key=1A3")

  # Using the Nasdaq SDK
  nasdaqdatalink.get("WIKI/AMD", rows=5)
  ```
...
* Why would companies create SDKs when Python provides users with the requests library?
    ...
    **Answer:** When writing a Python script, using a Python SDK can often be cleaner and easier to understand and integrate in one's code. SDKs allow developers to use syntax and language features that often simplify and clean up code. A decent example of this is evidenced when comparing the URL request to Nasdaq with the `nasdaqdatalink.get('AAPL')` function. Both will extract historical stock; however, the `nasdaqdatalink.get` function is easier to use and looks cleaner.
...
* Another important parameter we need to define to fetch stock data is the time frame we want to use. We can set the `timeframe` parameter in minutes (`1Min`, `5Min`, `15Min`) or one day (`1Day`). We will create a variable called `timeframe` to set this parameter as `1Day`.
...
* We are all set! Let's fetch the current closing prices for `FB` and `TWTR` using the `alpaca.get_bars()` function. Alpaca Python SDK automatically converts the resulting JSON response to a Pandas DataFrame if you use the `df` property.
...
* Though the `df` property produces a Pandas DataFrame, it must be reorganized slightly for optimal use by hierarchically indexing the tickers as the following code illustrates:
...
Explain to students that the `get_bars()` function from the `Alpaca` SDK takes in the following parameters that can be used to refine the query results: `symbols`, `timeframe`, `limit`, `start`, `end`, `after`, and `until`.

Be sure to point out that `limit` is set to the maximum value of 10,000 observations by default (i.e. no matter what time frame is specified, only the most recent 10,000 observations will be pulled by default unless a different value for limit is specified). We will refrain from setting a limit value to avoid exclusion of necessary data, however Students might find they want to change limit to a smaller number if they are working for instance with data at the minute frequency and want to pull data as fast as possible (e.g. for algorithm trading at high frequencies).
...
* We take advantage of the benefits of using an SDK, and we create a new DataFrame using the `alpaca.get_bars()` method by modifying the start and end dates.
...
* Note that because the DataFrame is multi-indexed, the `FB` and `TWTR` closing prices can be extracted from the `df_portfolio_year` DataFrame by using column keys.
...
* We will fetch daily data, so we set a `timeframe` variable as `1Day` to fetch closing prices on a daily basis.
...
* We retrieve the current closing prices using the `get_bars()` function from the Alpaca SDK and passing the `tickers`, `timeframe`, and `today` variables as parameters.
...
* With the right parameterization, the Nasdaq Data Link and Alpaca APIs could easily evolve into engines and frameworks.
