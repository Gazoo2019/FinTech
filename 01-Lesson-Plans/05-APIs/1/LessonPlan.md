...
* Slack out the [Alpaca and Nasdaq Data Link Installation Guide](../Supplemental/AlpacaMarkets_Installation-Guide.md). Tell students to complete the installation and verify it with a TA before the end of the next class. This should help catch installation issues with Alpaca outside of class time.

* This class will focus on the fundamentals of APIs and the process of submitting APIs. This class is a great opportunity for students to be impressed and inspired by FinTech. There are a number of fun and creative FinTech APIs, such as Nasdaq Data Link and the World Bank, will be used to drive student engagement. This lesson is the perfect opportunity to get students excited about FinTech again!

* The first half of the lesson will include instructor demonstrations of submitting requests to different FinTech APIs using [Postman](https://www.postman.com/downloads). Each demo should be focused on showcasing FinTech APIs and getting students excited about contributing to the wealth of APIs that are already out there. Make sure to create accounts for these APIs:
...
* The slides for this lesson can be viewed on Google Drive here: [5.1 Slides](https://docs.google.com/presentation/d/1u7Pwihvx4FMM3i9o8HeKezbbioKIpJzlmK-mHSu1kDY/edit?usp=sharing).
...
* [Nasdaq Data Link API Features](https://docs.data.nasdaq.com/docs/python-installation)
...
* There are a large number of FinTech APIs available that accelerate day-to-day financial analytics and data acquisition. These include Nasdaq Data Link and Wrap API, just to name two.

Introduce Nasdaq Data Link and briefly demonstrate to students how to use an internet browser to submit a Nasdaq Data Link API call.

* Nasdaq Data Link is a data mart of financial data. Nasdaq collects data from various sources, consolidates the data, and then makes it available to users. Nasdaq is a great product to use to extract financial data to calculate ROI, risk-to-reward ratio, etc. Users can access Nasdaq's data via their API. The API supports multiple programming languages, including Python.

Navigate to the [Nasdaq API Data Usage](https://docs.data.nasdaq.com/docs/in-depth-usage-1) page.

Show students the URL used to submit a Nasdaq Data Link API request. Underscore the fact that API requests work just like regular URLs. Also indicate that the **request URL** can be configured to specify the output format (e.g., CSV, JSON, or XML); highlight that JSON will be used for this class.

Submit a request to the Nasdaq API to get OPEC oil prices using an internet browser and the link below. Show students the data, and bring attention to the OPEC oil prices in the output. Note that you need to include your Nasdaq API key as a parameter in the URL to make the API call work.

```text
https://data.nasdaq.com/api/v3/datasets/OPEC/ORB.json?api_key=[YOUR-KEY-HERE]
```

  ![nasdaq_request_output.png](Images/nasdaq_request_output.png)

Finish the demo of Nasdaq Data Link with the following reflective questions:

* We've extracted historical stock data in the past using either the Nasdaq website or Google Finance. How does using the `Datalink` API compare? How is it different?

  **Answer:** Datalink is easier and quicker to use. As long as the URL is known, fewer steps are needed. A limitation is that the request URL has to be known.

* If you wanted to automate the extraction of historical stock prices, what's the better tool: something like nasdaq.com or Nasdaq Data Link?

  **Answer:** Nasdaq Data Link would be the best tool to use to automate the extraction of historical stock data. It would be easier than automating the nasdaq.com process.

If time remains, communicate to students that if they wanted to automate extraction of data from websites like the Nasdaq stock page, they could use the cool [WrapAPI](https://wrapapi.com/). Open the site and show students the Yelp GIFs.
...
* WrapAPI could be used to submit Datalink API requests via a web browser as well. WrapAPI could automate the process of copying and pasting the request URL into the browser.
...
In this activity, students will learn how to use Postman. The instructor will demo, making Nasdaq Data Link API calls with Postman rather than through a browser. This will highlight the advantages of using Postman, such as access to a UI, request logs, and tools that format API output for ease of use.
...
* We submitted Nasdaq Data Link API requests earlier in the class. What tool did we use to submit the API call?
...
Open Postman and perform a live demonstration of submitting an API request to Nasdaq Data Link for AAPL stock data. Use this as an opportunity to bridge back to the previous assignments where requests were submitted via a browser rather than a tool like Postman. Highlight the following:
...
* Postman allows users to save API requests to what they call **collections**. This allows requests to be created once but run many times. It also allows requests to be scheduled. In order to save a request, a request must be saved with a name, description, and collection.
...
* Submit an API call to Nasdaq Data Link using the Postman submit button. Use the below request URL.
...
In the next activity, students will leverage the steps from this demonstration in order to make their first API call using Postman. Students will make a Get request to **Nasdaq Data Link** for **AAPL** historical stock data.
...
Open Postman and submit the below request to Nasdaq Data Link. Then, conduct a live deciphering of the JSON data, emphasizing the following:
...
* JSON Arrays are identified by brackets `[]`. An example of a JSON Array is the `column_names` object returned from the Nasdaq API call.

If time permits, ask the students the following questions. Sample answers have been provided.

* We've seen a number of APIs in action (e.g., WrapAPI, Nasdaq, World Bank, Coinbase). Which API did everyone find the most interesting?

  **One Potential Answer:** WrapAPI. While it is not FinTech related, it's unique and has a number of different use cases. Of all of the APIs, WrapApi seems the most flexible and all-encompassing.
...
In this activity, students are given a list of **request URLs** to execute using the Python `requests` library. Students will also receive the opportunity to put their JSON knowledge to use by interpreting JSON output. Students will interpret the JSON output and store data from the output into variables.

Walk around with TAs to provide assistance to students with parsing JSON data. Students may need assistance parsing JSON output. Make sure every student has been able to successfully execute a request.
...
In this activity, go round-robin, asking students to do one of the following:
...
* Some of the APIs used in the lesson were Nasdaq Data Link, World Bank, Numbers API, Deck of Cards, U.S. Spending Stats,Bitcoin Pricing, etc. What APIs did you find to be the most interesting?
