...
* The slides for this lesson can be viewed on Google Drive here: [7.3 Lesson Slides](https://docs.google.com/presentation/d/1Lkl3Y3uPVmPGlEu5AzVGJY6FYhWVZVBEZ1Zg1fnCGI4/edit?usp=sharing).
...
Tell students that the SQLAlchemy library allows interaction between PostgreSQL and Python. Students should install this library on their virtual environment by executing the following commands in the terminal:
...
* [rental-erd.png](Images/rental-erd.png)
...
Slack out [rental-erd.png](Images/rental-erd.png) to the class and open it on your computer. Point out how each table has a connection to at least one other table. For example:

* The `customer` and `inventory` tables both contain `store_id` values.

* The `staff` and `rental` tables both contain `staff id` values.
...
* From the Export tab, select PostgreSQL from the drop-down menu. A file named `QuickDBD-export.sql` or something similar will appear in your Downloads folder.
  ...
  **Note:** The contents of the downloaded file are included in [`query.sql`](Activities/11-Stu_ERD/Solved/query.sql) if needed.
...
* Execute the code and verify that the tables have been created using a `SELECT` statement for each table.
